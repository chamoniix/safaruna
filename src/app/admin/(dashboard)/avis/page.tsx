...
export default function AdminReviewsPage() {
  ...
  async function moderate() {
    ...
    setSaving(true)
    const response = await fetch('/api/admin/reviews', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reviewId: editing.id, reviewType: editing.reviewType, status: nextStatus, reason }) })
    const data = await response.json().catch(() => ({}))
  ...
  return <div style={{ display: 'grid', gap: 18 }}>
    ...
    {visible.map(review => <article key={`${review.reviewType}:${review.id}`} style={{ padding: 20, background: 'white', border: '1px solid #E8DFC8', borderRadius: 14 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div><div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}><strong style={{ color: '#1A1209' }}>{review.guideName || 'SAFARUMA'}</strong><span style={{ padding: '4px 8px', borderRadius: 999, background: '#F7EEDB', color: '#8A6419', fontSize: 10, fontWeight: 800 }}>{review.label}</span></div><div style={{ color: '#7A6D5A', fontSize: 12 }}>{review.reservationRef ? `${review.reservationRef} · ` : ''}{review.author} · {[review.city, review.country].filter(Boolean).join(', ') || 'Localisation non renseignée'} · {review.authorEmail}</div></div><span style={{ alignSelf: 'start', padding: '6px 10px', borderRadius: 999, background: colors[review.status].bg, color: colors[review.status].fg, fontSize: 12, fontWeight: 800 }}>{labels[review.status]}</span></div><div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 14, color: '#9A6C14', fontWeight: 800 }}><Star size={18} fill="#C9A84C" color="#C9A84C" />{review.ratingOverall}/5</div><p style={{ color: '#4A3F30', lineHeight: 1.65 }}>{review.comment}</p>{review.reviewType === 'GUIDE' && <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', color: '#7A6D5A', fontSize: 12 }}>{review.ratingPunctuality !== null && <span>Ponctualité {review.ratingPunctuality}/5</span>}{review.ratingPedagogy !== null && <span>Pédagogie {review.ratingPedagogy}/5</span>}{review.ratingKnowledge !== null && <span>Connaissances {review.ratingKnowledge}/5</span>}{review.stayRating !== null && <span>Séjour {review.stayRating}/5</span>}</div>}{review.moderationNote && <div style={{ marginTop: 12, padding: 10, background: '#F8F6F2', borderRadius: 8, color: '#5B5145', fontSize: 12 }}><ShieldCheck size={14} style={{ verticalAlign: 'middle', marginRight: 5 }} />{review.moderationNote} — {review.moderatedByEmail}</div>}<button type="button" onClick={() => { setEditing(review); setNextStatus(review.status === 'APPROVED' ? 'HIDDEN' : 'APPROVED'); setReason('') }} style={{ marginTop: 15, border: 0, borderRadius: 999, padding: '9px 15px', background: '#1A1209', color: '#F0D897', fontWeight: 800, cursor: 'pointer' }}>Modérer</button></article>)}
