...
const GUIDE_CONFIRMATION_CONFIG = {
  NONE: { label: 'Non applicable', color: '#6B7280', bg: '#F3F4F6' },
...
export default function AdminReservations() {
  ...
  const stats = [
    ...
    { label: 'Guides en attente',  value: reservations.filter(r => r.guideConfirmationStatus === 'PENDING' || r.guideConfirmationStatus === 'PARTIAL').length, color: '#D97706', bg: '#FEF3C7' },
                  ...
                  return (
                    <tr key={r.id} style={{ background: i % 2 === 0 ? 'white' : '#FAFAF8', borderBottom: '1px solid #F0EBE0' }}>
                      ...
                      <td style={{ padding: '0.75rem 0.875rem' }}>
                        <span title={r.missions.map(mission => `${mission.city}: ${mission.guide} — ${mission.status}`).join('\n')} style={{ display: 'inline-block', background: gc.bg, color: gc.color, fontSize: '0.6rem', fontWeight: 700, padding: '0.25rem 0.6rem', borderRadius: 20, whiteSpace: 'nowrap' }}>{gc.label}</span>
                      </td>
                      <td style={{ padding: '0.75rem 0.875rem' }}>
