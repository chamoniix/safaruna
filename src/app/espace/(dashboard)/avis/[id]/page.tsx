...
export default function ReviewPage() {
  ...
  useEffect(() => {
    fetch(`/api/espace/reviews?reservationId=${encodeURIComponent(reservationId)}`, { cache: 'no-store' })
      .then(async response => {
        ...
        setFirstName(reservation.author?.firstName || '')
        setCity(reservation.author?.city || '')
        setCountry(reservation.author?.country || '')
  ...
  async function submit(event: React.FormEvent) {
    ...
    const response = await fetch('/api/espace/reviews', {
      ...
      body: JSON.stringify({ reservationId, firstName, city, country, stayRating, stayComment, guideReviews }),
    })
  ...
  return <form onSubmit={submit} style={{ display: 'grid', gap: 18, maxWidth: 820 }}>
    ...
    <section style={{ padding: 22, background: 'white', border: '1px solid #E8DFC8', borderRadius: 15 }}><h2 style={{ marginTop: 0, color: '#1A1209', fontSize: 20 }}>Votre signature publique</h2><p style={{ marginTop: -6, color: '#756B5D', fontSize: 13 }}>Seuls votre prénom, votre ville et votre pays seront affichés avec l’avis.</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}><label style={{ display: 'grid', gap: 7 }}><span style={{ color: '#4A3F30', fontSize: 13, fontWeight: 700 }}>Prénom</span><input required minLength={2} maxLength={80} disabled={!editable} value={firstName} onChange={event => setFirstName(event.target.value)} style={{ padding: 12, border: '1px solid #D8CEBC', borderRadius: 10, font: 'inherit' }} /></label><label style={{ display: 'grid', gap: 7 }}><span style={{ color: '#4A3F30', fontSize: 13, fontWeight: 700 }}>Ville</span><input required minLength={2} maxLength={120} disabled={!editable} value={city} onChange={event => setCity(event.target.value)} style={{ padding: 12, border: '1px solid #D8CEBC', borderRadius: 10, font: 'inherit' }} /></label><label style={{ display: 'grid', gap: 7 }}><span style={{ color: '#4A3F30', fontSize: 13, fontWeight: 700 }}>Pays</span><input required minLength={2} maxLength={120} disabled={!editable} value={country} onChange={event => setCountry(event.target.value)} style={{ padding: 12, border: '1px solid #D8CEBC', borderRadius: 10, font: 'inherit' }} /></label></div></section>
