import {
  Activity, AlertTriangle, ArrowLeft, ArrowRight, BarChart3, CheckCircle2, CircleDollarSign,
  Clock3, Cloud, CreditCard, Database, Eye, Gauge, Globe2, LayoutDashboard, ListFilter,
...
const funnelLabels: Record<string, string> = {
  ...
  begin_checkout: 'Checkout demandé', checkout_created: 'Arrivée Stripe', purchase: 'Paiement confirmé',
}
...
const eventLabels: Record<string, string> = {
  ...
  checkout_created: 'Session Stripe', checkout_error: 'Erreur checkout', payment_cancelled: 'Paiement annulé',
...
const viewTitles: Record<DashboardView, { eyebrow: string; title: string; description: string }> = {
  ...
  guides: { eyebrow: 'Marketplace', title: 'Guides', description: 'Profils consultés, guides actifs et nouvelles candidatures.' },
...
function Dashboard({ data, ga4, bigQuery, loginHistory, days, query, view }: { data: AnalyticsData; ga4: Ga4RealtimeData; bigQuery: BigQueryUsage | null; loginHistory: AdminLoginEvent[]; days: number; query: string; view: DashboardView }) {
  ...
  const guides = () => <><section className="metrics four"><Metric icon={<UserRound />} label="Guides actifs" value={number(data.overview.guidesActive)} note="visibles sur le site" tone="green" /><Metric icon={<Clock3 />} label="À examiner" value={number(data.overview.guidesPending)} note="candidatures en attente" tone="gold" /><Metric icon={<MousePointerClick />} label="Candidatures" value={number(data.overview.guideApplications)} note={`${days} derniers jours`} tone="teal" /><Metric icon={<Eye />} label="Profils vus" value={number(data.breakdowns.events.find(row => row.label === 'guide_viewed')?.count || 0)} note="événements enregistrés" /></section><section className="grid two"><article className="panel"><PanelTitle eyebrow="Intérêt" title="Guides les plus consultés" icon={<Eye />} /><RankedList rows={data.breakdowns.guides} /></article><article className="panel"><PanelTitle eyebrow="Candidatures" title="Actions dans l’inscription guide" icon={<ListFilter />} /><RankedList rows={data.breakdowns.events.filter(row => row.label.startsWith('guide_application')).map(row => ({ ...row, label: eventLabels[row.label] || row.label }))} /></article></section></>

  const payments = () => <><section className="metrics four"><Metric icon={<CircleDollarSign />} label="Chiffre d’affaires" value={euro(data.overview.revenue)} note="confirmé" tone="green" /><Metric icon={<CreditCard />} label="Confirmées" value={number(data.payments.purchases)} note="paiements réussis" tone="teal" /><Metric icon={<AlertTriangle />} label="Erreurs" value={number(data.payments.errors)} note="checkout" tone="red" /><Metric icon={<Clock3 />} label="Expirées" value={number(data.payments.expired)} note="sessions Stripe" tone="gold" /></section><section className="panel"><PanelTitle eyebrow="Stripe + SAFARUMA" title="État du tunnel de paiement" icon={<CreditCard />} /><div className="metric-ribbon payment-ribbon"><div><span>Arrivées Stripe</span><b>{number(data.payments.checkoutCreated)}</b></div><div><span>Confirmés</span><b>{number(data.payments.purchases)}</b></div><div><span>Erreurs</span><b>{number(data.payments.errors)}</b></div><div><span>Annulés</span><b>{number(data.payments.cancelled)}</b></div><div><span>Expirés</span><b>{number(data.payments.expired)}</b></div></div></section><section className="panel"><PanelTitle eyebrow="Réservations" title="Dernières demandes" icon={<CreditCard />} /><div className="table-wrap"><table><thead><tr><th>Référence</th><th>Client</th><th>Destination</th><th>Voyageurs</th><th>Montant</th><th>Statut</th><th>Date</th></tr></thead><tbody>{data.payments.reservations.map(item => <tr key={item.refNumber}><td><code>{item.refNumber}</code></td><td>{item.pelerin.name || item.pelerin.email || '—'}</td><td>{item.selectedCities || '—'}</td><td>{item.nbPeople}</td><td>{euro(item.totalPrice)}</td><td><span className={`pill ${item.status.toLowerCase()}`}>{item.status}</span></td><td>{date(item.createdAt)}</td></tr>)}</tbody></table>{!data.payments.reservations.length && <div className="empty">Aucune réservation sur cette période.</div>}</div></section></>
  ...
  const contentByView: Record<DashboardView, () => React.ReactNode> = { overview, realtime, audience, acquisition, content, auth, guides, payments, errors, search: searchView, infrastructure }
...
export default async function Page({ searchParams }: { searchParams: Promise<{ days?: string; q?: string; view?: string; accountPage?: string }> }) {
  ...
  const [result, ga4, loginHistory, bigQuery] = await Promise.all([
    getAnalyticsData(days, query, accountPage).then(data => ({ data, error: null })).catch(error => ({ data: null, error: error instanceof Error ? error.message : 'Erreur inconnue' })),
