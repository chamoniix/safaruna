...
function fixture() {
  ...
  const db = { emailDelivery: {
    ...
    findMany: async ({ where, take }: { where: { id?: { in: string[] }; OR: { status: string; category?: { in: string[] } }[] }; take: number }) => rows.filter(row =>
      row.payloadEncrypted && row.attempts < 3 && (!where.id || where.id.in.includes(row.id)) && where.OR.some(option =>
    ...
    updateMany: async ({ where, data }: { where: { id: string; status: string; attempts: number }; data: { status: string } }) => {
      ...
      row.status = data.status; row.attempts++; row.updatedAt = new Date(); return { count: 1 }
    },
  ...
  vm.runInNewContext(ts.transpileModule(readFileSync('src/lib/email.ts', 'utf8'), {
    ...
    module: sandboxModule, exports: sandboxModule.exports, Date, Error, console: { error: () => {} },
  ...
  return { ...sandboxModule.exports, rows, sent, queue, stored, due,
